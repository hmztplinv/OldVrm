...
if (get('complaint')) {
    ...
    //check if category tedarik, then change mailcontent
    if($complaint['category']=='Şikayet/Tedarik'){
        $mailtext='<p>Değerli müşterimiz '.$id.' nolu şikayetiniz sonuçlanmıştır.</p><p>Ürüne '.$complaint['sealeravaible'].' bayimizden ulaşabilirsiniz.</p><p>Şikayet sonuç raporunuz ektedir.</p><p>Bizi tercih ettiğiniz için teşekkür ederiz. </p><p>Sistemimizi geliştirmek için 6 soruluk
            kısa anketimize katılmak için aşağıdaki butona tıklayabilirsiniz.</p><p>Sağlıklı günler dileriz.</p>';

    }else{
        $mailtext='<p>Değerli müşterimiz '.$id.' nolu şikayetiniz sonuçlanmıştır. Şikayet sonuç raporunuz ektedir.</p><p>Bizi tercih ettiğiniz için teşekkür ederiz. </p><p>Sistemimizi geliştirmek için 6 soruluk
            kısa anketimize katılmak için aşağıdaki butona tıklayabilirsiniz.</p><p>Sağlıklı günler dileriz.</p>';
    }
    //end
...
';
    ...
    if ( $complaint["processConfirm"] ==1) {
        ...
        $pdf->MultiCell(80, 8, $txt1, '', 'R', '', 2, 105, 160, true);
    ...
    if ( $complaint["factoryConfirm"] ==1) {
        ...
        $pdf->MultiCell(80, 8, $txt2, '', 'R', '', 2, 190, 160, true);
    }
...
';
    ...
    $features = [
        ...
        'mailSubject' => ($companyid==2 ? 'bien':'qua').'.mutlumusteri.org '.$complaint['id'].' Numaralı Şikayet Sonucu',
...
if(!empty(complaint::complaintduration_exist($complaint['id']))){
    complaint::update_complaintduration($complaint['id']);
}
complaint::complaint_owner_mail_sent($complaint['id']);
...
if($companyid==2){
    ...
    sms::create_sms_log_complaint_sent_by_complaintid($complaint['phone'],$complaint['id']);
}

Complaint::update_complaint_status($complaint['id'],1);
header("Location: " . site_url('detail_complaint?id=' . base64_encode($complaint['id'])));
?>
