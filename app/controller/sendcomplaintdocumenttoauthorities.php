...
';
    ...
    if ( $complaint["processConfirm"] ==1) {
        if($companyid==2){
            ...
            $txt1 = "Bu belge Hasan Hüseyin Topoz  tarafından dijital ortamda onaylanmıştır.";
        }
...
';
    ...
    //$pdfString = $pdf->Output(public_url('complaintDocs/file.pdf'), 'F');

    //$pdf->Output('f.pdf','S');
    $complaintid=$complaint['id'];
    $postmails=[];
    $emails=complaint::get_authpersons_email($complaintid);
    foreach ($emails as $email){
        array_push($postmails,$email['authorizedMail']);
    }
...
<div id="email">
    ...
    <table role="presentation" width="100%" style="text-align: center">
        ...
        <tr><p>'.$complaint["id"].' numaralı  şikayet sonuçlanmıştır. Şikayet sonuç raporu ektedir.</p><p>Sağlıklı günler dileriz.. </p>
        </tr>
    </table>
<br><br><br>
    <! Banner Row -->
    ...
    if(get('pics')){
        ...
        $pics=json_decode(urldecode($_GET['pics']), true);

        if(!empty($pics['dataIds'])){
            $features['pics']=$pics['dataIds'];
        }
        if (!empty($pics['reportIds'])){
            $features['picsRep']=$pics['reportIds'];

        }
        if (!empty($pics['datavideoIds'])){

            $features['videos']=$pics['datavideoIds'];

        }
    }
    mail::send($features);
}
complaint::auth_mail_sent($complaint['id']);
