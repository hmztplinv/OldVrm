<?php require 'mainPage_statics/header.php'; ?>
<div class="container-fluid bg-light pt-5 p-3 small animate__animated animate__fadeIn">

    <ol class="breadcrumb small pt-5">
        <li class="breadcrumb-item text-custom">İK</li>
        <li class="breadcrumb-item text-custom"><a href="" class="text-decoration-none text-custom">GEÇ GELME VE ERKEN ÇIKIŞ BİLDİRİM FORMU</a></li>
    </ol>
    <div class="row pt-2" style="">
        <div class="col-12 col-sm-12 mb-3">
            <div class="card h-100">
                <header class="card-header d-md-flex align-items-center bg-custom2">
                    <h6 class="card-header-title text-light mt-1">GEÇ GELME VE ERKEN ÇIKIŞ BİLDİRİM FORMU</h6>
                </header>
                <div class="container-fluid" >
                    <div class="col-sm-6 mx-auto mt-4" >
                        <div class="row w-25 pr-0 float-right">
                            <div class="col-sm-6" style="background-color: green;color: white;font-size: 22px;text-align: center"><label style="margin-top: 15px">34</label></div>
                            <div class="col-sm-6" style="text-align: center;background-color: lightgrey"><label style="  font-size: 22px;margin-top: 15px">11</label></div>
                        </div>
                    </div>
                    <div class="col-sm-12">
                        <img style="clear: both; width: 200px;height: 120px;display: block;margin: auto" src="data:image/png;base64," alt="..." class=" ">
                    </div>
                    <div class="col-sm-6 text-center mx-auto p-0">
                        <header class="card-header  bg-custom2">
                            <h6 class="card-header-title text-light mt-1">GEÇ GELME VE ERKEN ÇIKIŞ BİLDİRİM FORMU</h6>
                        </header>
                    </div>
                    <div class="col-sm-5 col-10 text-center mx-auto">
                        <div class="row h-100" >
                            <div class="col-md-12">
                                <table class="table table-bordered mt-4">

                                    <tbody>
                                    <tr>
                                        <th scope="row" style="width: 30%">Adı Soyadı :</th>
                                        <td>Damla Selcen Çatal</td>


                                    </tr>

                                    <tr>
                                        <th scope="row">Kart No :</th>
                                        <td colspan="2">00000447 </td>

                                    </tr>
                                    <tr>
                                        <th scope="row">Departman :</th>
                                        <td colspan="2">Muhasebe Müdürlüğü</td>

                                    </tr>
                                    <tr>
                                        <th scope="row">Görev :</th>
                                        <td colspan="2">Muhasebe Uzman Yardımcısı </td>

                                    </tr>
                                    <tr>
                                        <th scope="row">Açıklama :</th>
                                        <td colspan="2">Sn. Damla Selcan Çatal, 11.10.2022 tarihinde ki mesainizden 16 dakika erken çıkış yaptığınız tespit edilmiştir. Konuyla ilgili açıklamanızı yazmanızı rica ederim.</td>

                                    </tr>
                                    </tbody>
                                </table>
                            </div>
                            <div class="col-md-4">
                                <table class="table table-bordered h-75">
                                    <tr>
                                        <th scope="col" style="width: 50%; padding: 20px 0px;">Geç Giriş</th>
                                        <th scope="col" style="width: 50%"> </th>
                                    </tr>


                                </table>
                            </div>
                            <div class="col-md-4">
                                <table class="table table-bordered h-75">
                                    <tr>
                                        <th scope="col" style="width: 50%; padding: 20px 0px;">Erken Çıkış</th>
                                        <th scope="col" style="width: 50%; background-color: lightcoral; padding: 7px 0px;"> <i class="bi bi-x text-danger h1"></i></th>
                                    </tr>
                                </table>
                            </div>
                            <div class="col-md-4">
                                <table class="table table-bordered">
                                    <tr>
                                        <th scope="col" style="width: 50%">Geç Giriş / Erken Çıkış</th>
                                        <th scope="col" style="width: 50%;"></th>
                                    </tr>
                                </table>
                            </div>
                        </div>
                    </div>
                    <div class="col-sm-5 col-10 text-center  mx-auto p-0">
                        <div class="col-sm-6 float-right mb-3">
                            <table class=" border w-100">
                                <tbody>
                                <tr>
                                    <td style="height: 75px"></td>
                                </tr>
                                <tr>
                                    <td style="">Mehmet BİLGİN
                                    </td>
                                </tr>
                                <tr>
                                    <td colspan="2">İnsan Kaynakları Müdürü</td>
                                </tr>
                                </tbody>
                            </table>
                        </div>
                    </div>
                    <div class="col-sm-6 text-center mx-auto p-0 mt-3" style="clear: both;">
                        <header class="card-header  bg-custom2">
                            <h6 class="card-header-title text-light mt-1">Mesai Giriş Çıkış Bilgileri</h6>
                        </header>
                    </div>
                    <div class="col-sm-5 col-10 text-center mx-auto mt-3">
                        <div class="row" >
                            <div class="col-md-5">
                                <table class="table table-bordered">
                                    <tr>
                                        <th scope="col" style="width: 50%">Giriş Tarihi</th>
                                        <th scope="col" style="width: 50%">1.10.2023 </th>
                                    </tr>
                                </table>
                            </div>
                            <div class="col-md-7">
                                <table class="table table-bordered">
                                    <tr>
                                        <th scope="col" style="width: 34%">Giriş Çıkış Saati :</th>
                                        <th scope="col" style="width: 33%"> 08:16</th>
                                        <th scope="col" style="width: 33%"> 17:44</th>
                                    </tr>
                                </table>
                            </div>
                        </div>
                    </div>
                    <div class="col-sm-6 text-center mx-auto p-0">
                        <header class="card-header  bg-custom2">
                            <h6 class="card-header-title text-light mt-1">Açıklama</h6>
                        </header>
                    </div>
                    <div class="col-sm-5 col-10 text-center mx-auto mt-3">
                        <p>.......................................................................................................................................................................................................................................................</p>
                        <p>.......................................................................................................................................................................................................................................................</p>
                        <p>.......................................................................................................................................................................................................................................................</p>
                        <p>.......................................................................................................................................................................................................................................................</p>
                        <p>.......................................................................................................................................................................................................................................................</p>
                        <p>.......................................................................................................................................................................................................................................................</p>
                        <p>Ad Soyad <br> İmza</p>
                    </div>

                </div>
            </div>
        </div>
    </div>
</div>
</body>

<?php require 'mainPage_statics/footer.php'; ?>
