...
<div class="container-fluid bg-light pt-5 p-3 small animate__animated animate__fadeIn">
    ...
    <!--   // toplam şikayet // açık şikayet // ortalama şikayet kapatma süresi (gün)// çözülen şikayetler
       //en çok şikayet alınan bayiler // en çok şikayet alınan ürünler // -->
    <!-- Row 1 -->
    <div class="row pt-2">
        ...
        <div class="col-6 col-sm-3 mb-3">
            <div class="card h-100">
                ...
                <?php
                $query1=$db->prepare("SELECT COUNT(*) AS count FROM `hrEmployee` INNER JOIN auth ON hrEmployee.registry=auth.userid WHERE YEAR(endDate) = YEAR(CURDATE()) AND recordStatus = 1");
                $query1->execute();
    ...
    <div class="row pt-2">
        ...
        <div class="col-12 col-sm-12 mb-3">
            <div class="card h-100">
                <header class="card-header d-md-flex align-items-center bg-custom2">
                    <h6 class="card-header-title text-light my-auto">Aktif Personel Listesi</h6>
                    <a href="<?=site_url('personel_ekleme')?>" class="ml-auto">
                        <button type="button" class="btn btn-light">Ekle</button>
                    </a>
                </header>
                <div class="card-body">
...
<script>
    ...
    var myChart = new Chart(ctx, {
        ...
        data: {
            labels: [
                <?php
                $query1=$db->prepare("SELECT hrEmployeeDepartment.department, COUNT(*) as count FROM hrEmployeeDepartment WHERE positionActivePassive = 'A' AND departmentStatus = 1 GROUP BY department ORDER BY count DESC");
                $query1->execute();
...
<script>
    ...
    var myChart = new Chart(ctx, {
        ...
        data: {
            labels: [
                <?php
                $query1=$db->prepare("SELECT positionTitle, COUNT(*) as count FROM hrEmployeeDepartment INNER JOIN auth ON hrEmployeeDepartment.registry = auth.userid  WHERE   positionActivePassive = 'A' AND departmentStatus = 1 GROUP BY positionTitle ORDER BY count DESC");
                $query1->execute();
                while($query1r=$query1->fetch(PDO::FETCH_ASSOC)) {?>
            ...
            datasets: [{
                ...
                data: [
                    <?php
                    $query1=$db->prepare("SELECT positionTitle, COUNT(*) as count FROM hrEmployeeDepartment INNER JOIN auth ON hrEmployeeDepartment.registry = auth.userid  WHERE auth.companyid=:companyid AND  positionActivePassive = 'A' AND departmentStatus = 1 GROUP BY positionTitle ORDER BY count DESC");
                    $query1->execute([
...
<script>
    ...
    var myChart = new Chart(ctx, {
        ...
        data: {
            labels: [
                <?php
                $query1=$db->prepare("SELECT (CASE WHEN sex='K' THEN 'Kadın' WHEN sex='E' THEN 'Erkek' ELSE 'Hata' END) AS sex, COUNT(*) AS count FROM hrEmployee INNER JOIN auth ON hrEmployee.registry=auth.userid WHERE auth.companyid=:companyid AND ActivePassive = 'A' AND RecordStatus = 1 GROUP BY sex ORDER BY count DESC");
                $query1->execute([
                    'companyid'=>session('company_id')
                ]);
                while($query1r=$query1->fetch(PDO::FETCH_ASSOC)) {?>
