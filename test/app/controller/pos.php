...
$pos = api::API_url('https://test-api.finekra.com/api/PosTransaction');
$list_bank = [];
$list_member = [];
...
for ($i=1;$i<=12;$i++){
    if ($i == 1){
        ...
        $currentMonth[1] = api::API_url("https://test-api.finekra.com/api/PosTransaction?\$filter=date(valueDate)%20gt%20".$currentMonthStart."%20and%20date(valueDate)%20lt%20".$currentMonthEnd);
    }else{
        ...
        if (in_array(date("m",strtotime("+".$i."month")),$climbMonth)){
            ...
            $currentMonth[$i] = api::API_url("https://test-api.finekra.com/api/PosTransaction?\$filter=date(valueDate)%20gt%20".$currentMonthStart."%20and%20date(valueDate)%20lt%20".$currentMonthEnd);
        }else{
...
if (post('bankName')  || post('member') || post('startDate') || post('endDate') || post('startAmount') || post('endAmount')){
    ...
    //damp($urlString);
    $pos = api::API_url('https://test-api.finekra.com/api/PosTransaction'.$urlString);
...
if (post('changeMember')){
    ...
    $posd = api::API_url('https://test-api.finekra.com/api/PosTransaction'.$urlString);
    $listMember = [];
...
if (post('changeBank')){
    ...
    $cBank = api::API_url('https://test-api.finekra.com/api/PosTransaction'.$urlString);
    $listBank = [];
